...
public:
    ...
    int numNodes() const;
    string findsecondlargest();
    

private:
    // Declare a root node
    AVLNode* root;
    string findsecondlargest(AVLNode* n);
    ...
    void insert(AVLNode*& current, const string& x);
    //helper method for pathTo
    void pathTo(const string& x, AVLNode* current, vector<string>& arr);
    //helper method for find
    bool find(const string& x, AVLNode* current)const;
    //helper method for numNodes
    int numNodes(AVLNode* current)const;
};
